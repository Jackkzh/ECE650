...
void * ff_malloc(size_t size) {
    // 1. check if there's available free blocks to use
    meta_info * curr = NULL;
    if (free_block_head != NULL) {
        curr = free_block_head;
        for (curr = free_block_head; curr != NULL; curr = curr->next) {
            // if there is an available option 
            if (curr->size >= size) {
                curr->isUsed = true;
                if (curr->size > size + sizeof(meta_info)) {
                    meta_info * splitted = splitBlock(curr, size);
    ...
    // 2. create a new block for malloc - using sbrk()
    if (curr == NULL) {
...
/*
    bf_free uses the same strategy as ff_free
*/
